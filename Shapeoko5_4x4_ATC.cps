...
if (!newWorkPlane && insertToolCall && (getProperty("useATC"))){
  writeBlock("(T" + getPreviousSection().getTool().number + " Tool Drop Off)");
  writeBlock("G4 P.5");
  writeBlock("G0 G53 Z-.197");
  writeBlock("G0 G53 " + "Y" + yTcPos[getPreviousSection().getTool().number] + " X-3.937");
  writeBlock("G0 G53 " + "X" + Xpos);
  writeBlock("G0 G53 Z-5.3125");
  writeBlock("G1 G53 Z" + TCZDropOff + " F15.");
  writeBlock("G4 P.5");
  writeBlock("G0 G53 Z-.197");
  }
///JDM Edit Add Tool Changer Pick Ups
if (insertToolCall && isFirstSection && (getProperty("useATC"))){
  writeBlock("(T" + tool.number + " Pick Up)");
  writeBlock("G0 G53 Z-.197")
  writeBlock("G0 G53" + " Y" + yTcPos[tool.number]);
  writeBlock("G0 G53 " + "X" + Xpos);
  writeBlock("G0 G53 Z-5.3125");
  writeBlock("G1 G53 Z" + TCZPickup + " F5.");
  writeBlock("G4 P1.");
  writeBlock("G0 G53 Z-.197");
  }
  ...
  if (insertToolCall) {
    ...
    if (tool.number > getProperty(numberOfToolSlots)) {
...
function onClose() {
  ...
  onImpliedCommand(COMMAND_END);
    ...
    if ((getProperty("useATC"))){
      writeBlock("(Final T" + tool.number + " Tool Drop Off)");
      writeBlock("G0 G53 Z-.197");
      writeBlock("G0 G53 X-3.937 " + "Y" + yTcPos[tool.number]);
      writeBlock("G0 G53 " + "X" + Xpos);
      writeBlock("G0 G53 Z-5.3125");
      writeBlock("G1 G53 Z" + TCZDropOff + " F15.");
      writeBlock("G4 P.5");
      writeBlock("G0 G53 Z-.197");
      writeBlock("G0 G53 X-3.937");
}
  writeRetract(X, Y);
